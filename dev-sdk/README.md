...
- **Default:** When the dev CLI is run with `--debug` (or `DEV_DEBUG=1`), it sets the `dev_sdk` logger to DEBUG and adds a **file handler** so all SDK debug logs go to a log file (not stdout). The log file path is `~/.config/dev/sdk-debug.log` (or the path given by `DEV_SDK_LOG`).
