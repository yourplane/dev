...
The SDK logs at **DEBUG** level around blocking operations (agent chat creation, git clone, branch checkout) so you can see where execution is when things hang. The logger name is `dev_sdk`.

- **Default:** The dev CLI enables SDK debug logging on every run: it sets the `dev_sdk` logger to DEBUG and adds a **file handler** so all SDK debug logs go to a log file (not stdout). The log file path is `~/.local/share/dev/sdk-debug.log` (or the path given by `DEV_SDK_LOG`).
