...
def pull_pr_comments(task_root: Path) -> tuple[str, int, str | None]:
    ...
    def _write_comms(markdown: str) -> str:
        out = add_comms(task_root, "agent", markdown, kind="pr-comments")
        return out.name

    try:
        result = lib_pull_comments(
            comms_dir(task_root),
            pr_url=pr_url,
            token=token,
            write_output=_write_comms,
        )
    except PrLibError as e:
        raise CreatePRError(str(e)) from e

    return result.pr_url, result.new_count, result.output_filename
